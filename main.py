...
def verifier_cycles_oracle():
    ...
    if len(local_session_history) < 3:
        return "⏳ **Synchronisation en cours...**\nL'Oracle accumule les premiers tirages réels. Réessayez dans 30 secondes."
    ...
    if not preparation_pure_sans_zero:
        return f"📉 **SÉQUENCE EN COURS : NEUTRE**\n*L'Oracle attend un cycle pur sans zéro.*\n\nManche précédente : `{t_actuel['cg']} {t_actuel['g']}` | `{t_actuel['cc']} {t_actuel['c']}` | `{t_actuel['cd']} {t_actuel['d']}`"
...
async def scraper_historique_1win_en_continu():
    """ Capteur Cloud Réparé : Décode correctement la liste d'objets JSON de 1win """
            ...
            if response.status_code == 200:
                ...
                if manches_recues:
                    ...
                    if id_manche not in historique_manches_traitees:
                        ...
                        if isinstance(cartes, list) and len(cartes) == 3:
                            # 🔒 REPARATION DES INDEX : Extraction chirurgicale des objets du tableau
                            g = int(cartes[0].get("value", 0))
                            c = int(cartes[1].get("value", 0))
                            d = int(cartes[2].get("value", 0))
